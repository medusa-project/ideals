...
namespace :import do
  ...
  task :items => :environment do
    ...
    File.open( Rails.root.join("lib", "assets", "import_sources", "items.csv"), "r").each_line do |line|
      ...
      if ["illinois.edu", "uis.edu", "uic.edu"].include?(email_parts[-1])
        submitter_auth_provider = Ideals::AuthProvider::SHIBBOLETH
      else
        submitter_auth_provider = Ideals::AuthProvider::IDENTITY
      end
      in_archive = row[2] == 't'
      withdrawn = row[3] == 't'
