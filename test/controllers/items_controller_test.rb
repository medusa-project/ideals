...
class ItemsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "edit_bitstreams() redirects to login page for logged-out users" do
    item = items(:item1)
    get item_edit_bitstreams_path(item), xhr: true
    assert_redirected_to login_path
  end

  test "edit_bitstreams() returns HTTP 403 for unauthorized users" do
    log_in_as(users(:norights))
    item = items(:item1)
    get item_edit_bitstreams_path(item), xhr: true
    assert_response :forbidden
  ...
  test "edit_bitstreams() returns HTTP 404 for non-XHR requests" do
    log_in_as(users(:admin))
    item = items(:item1)
    get item_edit_bitstreams_path(item)
    assert_response :not_found
  end

  test "edit_bitstreams() returns HTTP 200 for XHR requests" do
    log_in_as(users(:admin))
    item = items(:item1)
    get item_edit_bitstreams_path(item), xhr: true
    assert_response :ok
